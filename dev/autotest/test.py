...
def _run_fds(fds_file_path):
    ...
    end_file_path = fds_file_dir + "/" + fds_file_name.split(".")[0] + ".end"
    print(end_file_path) # FIXME
    subprocess.call(['fds',fds_file_path], cwd=fds_file_dir)
    # Check existance of .end file
    if not os.path.isfile(end_file_path): raise Exception("Error in FDS calculation:", fds_file_name)
